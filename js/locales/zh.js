...
I18n.register('zh', {
    ...
    info: {
        1: 'Arc 是 USDC 发行方 Circle 推出的开放式一层区块链。它兼容 EVM，专为稳定币金融而设计。目前 Arc 以公共测试网的形式运行——正是这款游戏所在的网络。',
        2: '在 Arc 上，Gas 用 USDC 支付，而不是波动的代币。手续费直接以美元计价，平均每笔约 0.005 美元，任何人都能像普通开支一样把它列入预算。',
        3: 'Arc 将 Malachite（Tendermint BFT 的 Rust 实现）与 Reth 执行层结合。在公共测试网上，出块间隔约为 0.5 秒，最终性是确定性的且在一秒以内：没有重组，也不必等待确认。',
        4: 'ARC 是网络的原生代币，但它不是 Gas 代币。以稳定币支付的协议手续费会自动转换为 ARC，然后在质押奖励与永久销毁之间分配。ARC 尚未发行。',
        ...
        15: '总得有人来给交易排序。Arc 从一组许可制验证者起步——由知名且可问责的机构运行节点——并随着时间推移逐步开放。这是一次有意的取舍：起步阶段验证者更少，换来的是你可以追责的具体名字。',
        16: '一笔跨境电汇要在多家代理行之间辗转，每家都有自己的截止时间，所以资金可能因为一个周末就闲置好几天。稳定币通道不看办公时间：同样一笔转账在几秒内完成结算，任何时刻都可以，而且手续费在发送前就已确定。',
        17: 'Arc 的测试网并不冷清。自 2025 年 10 月开放以来，已有一百多家机构——银行、支付公司、资产管理机构——在上面跑真实业务流。支付网络本就该这样测试：用别人真实的业务流程，在任何人真实的资金进入之前。',
        18: '已完成结算的转账没有撤单一说。转错地址，任何客服都追不回来——让结算变快的那种最终性，也让错误变得不可挽回。这正是消费限额、授权额度和测试网存在的理由：你只能事前检查，因为没有事后。',
        19: 'Arc 目前仍是公共测试网。主网测试版要等测试和监管工作完成之后才会到来，验证者集合会从那时起扩大，ARC 代币也还在前方。你一直是在一个正被建造的网络上游玩——而这是你唯一能亲眼看着它被建起来的时候。',
    ...
    quiz: {
        ...
        17: {
            question: '自 Arc 测试网开放以来，都是谁在使用它？',
            answers: [
                '没有人——那是 Circle 内部的私有演示',
                '一百多家机构，用测试资金跑真实的支付流程',
                '只有像这款游戏里的散户玩家'
            ]
        },
        18: {
    ...
    rescueTopics: {
        ...
        network: {
            title: 'Arc 究竟是什么样的网络',
            hint: 'Arc 是一层网络：它自己结算自己的区块，而不是把区块发布到别人的链上。它兼容 EVM，因此为以太坊编写的合约和工具无需改动即可运行。而它目前仍是公共测试网——真实的软件配上一文不值的资金，这恰恰是找出漏洞的最佳时机。',
            questions: [
                {
